...
class AttentionBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        print("kvq_cost = {}".format(kvq_cost))
        print("qk_cost = {}".format(qk_cost))
        print("mask_fill_cost = {}".format(mask_fill_cost))
        print("softmax_cost = {}".format(softmax_cost))
        print("weight_value_cost = {}".format(weight_value_cost))
        print()
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, key_size, num_sub_blocks, temp_attn, nheads, en_res, stride, dilation, 
                padding, vhdrop_layer, visual, dropout=0.2):
    ...
    def _make_layers(self, num_sub_blocks, n_inputs, n_outputs, kernel_size, stride, dilation, 
        ...
        # layers_list.append(AttentionBlock(n_inputs, key_size, n_inputs))
        layers_list.append(
    ...
    def forward(self, x):
        ...
        if self.temp_attn == True:
            ...
            # print("attn_cost = {}".format(attn_cost))
            # print("tcn_and_res_cost = {}".format(tcn_and_res_cost))
            # print("to_cpu_cost = {}".format(to_cpu_cost))
            # print()
...
class TemporalConvNet(nn.Module):
    def __init__(self, input_output_size, emb_size, num_channels, num_sub_blocks, temp_attn, nheads, en_res,
                key_size, kernel_size, visual, vhdropout=0.0, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            layers += [TemporalBlock(in_channels, out_channels, kernel_size, key_size, num_sub_blocks, \
                temp_attn, nheads, en_res, stride=1, dilation=dilation_size, padding=(kernel_size-1) * dilation_size, \
    ...
    def forward(self, x):
        ...
        if self.temp_attn:
            ...
            # if self.temp_attn_share is not None:
            #     out = x
            #     for i in range(len(self.network)):
            #         out = self.temp_attn_share(out)
            #         out, attn_weight = self.network[i](out)
            #         attn_weight_list.append(attn_weight[-1])
            #     return out, attn_weight_list
